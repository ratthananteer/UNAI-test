...
function belongsToFloor(item: Item, selectedId: string | number): boolean {
  const value = idOf(
    item.floor_id ?? item.floorId ?? item.floor ?? item.floorID,
  );
...
export default function BuildingLiveMap({
  ...
  const [internalFloorId, setInternalFloorId] = useState<
  ...
  const getUserName = (tag: Item): string => {
    const first = str(
      tag.firstname ?? tag.first_name ?? tag.firstName,
      "",
    ).trim();
    ...
    return (
      ...
      str(
        tag.ui_display ?? tag.label ?? tag.name ?? tag.tag_name ?? tag.tagName,
        "",
      ).trim() ||
  ...
  const searchableTags = useMemo(() => {
    ...
    const candidates = tags.filter((tag: Item) =>
      belongsToFloor(tag, selectedFloorId),
    );
    ...
    return candidates
      .filter((tag: Item) => {
        ...
        return (
          getUserName(tag).toLowerCase().includes(query) || id.includes(query)
        );
      })
      .slice(0, 8);
  ...
  const selectedUser = useMemo(() => {
    ...
    return tags.find(
      (tag: Item) =>
        String(tag.id ?? tag.tagId ?? tag.tag_id) === selectedUserTagId,
    );
  ...
  const selectedFloor = useMemo(
    () =>
      usableFloors.find(
        (floor) => String(floorId(floor)) === String(selectedFloorId),
      ),
    [usableFloors, selectedFloorId],
  );

  // Keep the group filter hook order stable. The filter is derived from the
  // same floor-scoped tag source passed into LiveMap.
  const liveTags = useMemo(
    () =>
      tags.filter(
        (item) =>
          selectedFloorId !== undefined &&
          tagBelongsToFloor(item, selectedFloorId),
      ),
    [tags, selectedFloorId],
  );
  const tagGroups = useMemo(() => {
    ...
    for (const tag of liveTags) {
      const groupId = tag.group_id ?? tag.groupId;
      const groupName = str(
        tag.group_name ?? tag.groupName ?? tag.group,
        "",
      ).trim();
      const name =
        groupName ||
        (groupId != null ? `Group ${String(groupId)}` : "Ungrouped");
      const key = groupId != null ? String(groupId) : name;
  ...
  useEffect(() => {
    const available = new Set(tagGroups.map((group) => group.key));
  ...
  const liveFloor = {
    id: selectedFloorId,
    name: str(
      floorValue("name") ?? floorValue("floor_name") ?? floorValue("title"),
      `Floor ${String(selectedFloorId)}`,
    ),
    map_path: (floorValue("map_path") ?? floorValue("mapPath") ?? null) as
      | string
      | null,
    map_width: num(
  ...
  const liveAnchors = anchors
    .filter((item) => belongsToFloor(item, selectedFloorId))
    .map((item) => ({
      id: idOf(item.id ?? item.anchor_id ?? item.anchorId),
      x: idOf(item.x ?? item.pos_x ?? item.position_x) as
        | number
        | string
        | null,
      y: idOf(item.y ?? item.pos_y ?? item.position_y) as
        | number
        | string
        | null,
  ...
  const liveZones = zones
    ...
    .map((item) => ({
      ...
      polygon_data:
        typeof item.polygon_data === "string" ? item.polygon_data : null,
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b bg-white p-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input
            ...
            value={tagIdFilter}
            onChange={(event) =>
              setTagIdFilter(event.target.value.replace(/\D/g, ""))
            }
            onKeyDown={(event) => {
              if (event.key === "Enter") {
          ...
          <button
            ...
            onClick={() => {
              ...
              const match = tags.find(
                (tag: Item) => String(tag.id ?? tag.tagId ?? tag.tag_id) === id,
              );
          ...
          <div className="relative">
            <button
              ...
              className={`rounded-lg border px-3 py-2 text-sm font-medium transition ${selectedGroupFilters.length ? "border-violet-400 bg-violet-50 text-violet-700" : "border-slate-300 bg-white text-slate-700"}`}
            ...
            {groupFilterOpen && (
              <div className="absolute right-0 top-full z-[70] mt-1 w-72 rounded-xl border border-slate-200 bg-white p-2 shadow-xl">
                ...
                {tagGroups.length === 0 ? (
                  <p className="px-2 py-4 text-xs text-slate-400">
                    No groups found on this floor.
                  </p>
                ) : (
                  <div className="mt-1 max-h-64 overflow-y-auto">
                    {tagGroups.map((group) => {
                      const checked = selectedGroupFilters.includes(group.key);
                      return (
                        <label
                          key={group.key}
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs">
              <span className="text-slate-500">
                ...
                >
                  {selectedUser.status === 1 ? "ONLINE" : "OFFLINE"}
                </strong>
              </span>
              <span className="text-slate-500">
                Group{" "}
                <strong className="text-slate-700">
                  {str(selectedUser.group_name ?? selectedUser.groupName, "—")}
                </strong>
              </span>
              <span className="text-slate-500">
                Position{" "}
                <strong className="text-slate-700">
                  X {str(selectedUser.x, "—")}, Y {str(selectedUser.y, "—")}
                </strong>
              </span>
              <span className="text-slate-500">
                Last seen{" "}
...
export function BuildingMapModes({
  ...
  const validFloors = useMemo(
    () => floors.filter((item) => floorId(item) !== undefined),
    [floors],
  );
  const [mode, setMode] = useState<"live" | "history">("live");
  const [selectedFloorId, setSelectedFloorId] = useState<
    string | number | undefined
  >(floorId(validFloors[0]));
  useEffect(() => {
    const exists = validFloors.some(
      (item) => String(floorId(item)) === String(selectedFloorId),
    );
    if (!exists) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b p-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Live tracking and tag history share the same floor. History shows
            all tags by default.
          </p>
        </div>
        <div className="flex rounded-lg border bg-slate-100 p-1">
          <button
            ...
            className={`rounded-md px-4 py-2 text-sm font-medium ${mode === "live" ? "bg-white text-blue-700 shadow-sm" : "text-slate-600"}`}
          >
            Live Map
          </button>
          <button
            ...
            className={`rounded-md px-4 py-2 text-sm font-medium ${mode === "history" ? "bg-white text-blue-700 shadow-sm" : "text-slate-600"}`}
          >
            Tag History
          </button>
        </div>
...
function BuildingTagHistory({
  ...
  const tagNames = useMemo(() => {
    ...
    for (const tag of tags) {
      ...
      const last = str(
        tag.lastname ?? tag.last_name ?? tag.lastName,
        "",
      ).trim();
      const name =
        [first, last].filter(Boolean).join(" ").trim() ||
  ...
  function userNameOf(event: HistoryEvent): string {
    return (
      tagNames.get(String(event.tagId ?? "")) ||
      event.tagName ||
      String(event.tagId ?? "")
    );
  }
  ...
  const selectedFloor = useMemo(
    () =>
      floors.find((item) => String(floorId(item)) === String(selectedFloorId)),
  ...
  const heatmapPoints = useMemo(() => {
    ...
    for (const event of history) {
      ...
      if (
        selectedFloorId == null ||
        String(event.floorId) !== String(selectedFloorId)
      )
        continue;
    ...
    const maxCount = Math.max(
      1,
      ...Array.from(bins.values()).map((point) => point.count),
    );
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () =>
        setIndex((value) => {
          if (value >= selectedHistory.length - 1) {
            setPlaying(false);
            return value;
          }
          return value + 1;
  ...
  useEffect(() => {
    ...
    if (
      eventFloor !== undefined &&
      eventFloor !== null &&
      String(eventFloor) !== String(selectedFloorId)
    )
      onFloorChange(eventFloor);
  ...
  if (!selectedFloor)
    return (
      <div className="p-6 text-sm text-slate-500">
        No floor available for this building.
      </div>
    );
  const width = num(selectedFloor.map_width ?? selectedFloor.mapWidth) ?? 1600;
  const height =
    num(selectedFloor.map_height ?? selectedFloor.mapHeight) ?? 900;
    ...
    <div>
      <div className="flex flex-wrap items-center gap-3 border-b p-4">
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") setTagId(followedTagId.trim());
          }}
          placeholder="Tag ID"
          inputMode="numeric"
        ...
        <button
          type="button"
          disabled={!followedTagId.trim()}
          onClick={() => setTagId(followedTagId.trim())}
        ...
        <select
          ...
          onChange={(e) => {
            ...
            setFollowedTagId(e.target.value);
            setIndex(0);
            setPlaying(false);
          }}
          className="rounded-lg border px-3 py-2 text-sm"
        ...
        >
          ...
          {Array.from(
            new Map(
              events
                .filter((e) => e.tagId !== undefined)
                .map((e) => [
                  String(e.tagId),
                  `${tagNames.get(String(e.tagId)) || e.tagName || `Tag ${e.tagId}`} (${e.tagId})`,
                ]),
            ).entries(),
          ).map(([id, label]) => (
        ...
        <select
          ...
          aria-label="Select floor"
        >
          {validFloorOptions(floors).map(([id, name]) => (
            <option key={id} value={id}>
              {name}
            </option>
          ))}
        ...
        <button
          ...
          onClick={() => {
            if (index >= selectedHistory.length - 1) setIndex(0);
            setPlaying((v) => !v);
          }}
        ...
        <span className="ml-auto text-xs text-slate-500">
          {tagId
            ? selectedHistory.length
              ? `${index + 1} / ${selectedHistory.length}`
      ...
      <div className="overflow-auto bg-slate-100 p-4">
        <div
          className="relative mx-auto max-w-[1000px] overflow-hidden rounded-xl border bg-white"
          style={{ aspectRatio: `${width}/${height}` }}
        >
          {map && (
            <img
              src={map}
              alt={String(selectedFloor.name ?? "Floor map")}
          ...
          >
            {zones
              .filter((zone) => {
                ...
                return (
                  zid === undefined || String(zid) === String(selectedFloorId)
                );
              })
              .map((zone, zi) => {
                const points = polygon(zone.polygon_data)
                  .map((point: unknown) => {
          ...
          {mapHistory.map((event, eventIndex) => {
            ...
            const tagEvents =
              historyByTag.find(([id]) => id === String(event.tagId))?.[1] ??
              [];
            ...
            const left = Math.max(
              0,
              Math.min(100, ((ox + px * scale) / width) * 100),
            );
            const top = Math.max(
              0,
              Math.min(100, ((oy - py * scale) / height) * 100),
            );
            const displayUserName = userNameOf(event);
            return (
              <div
                key={`history-point-${event._id ?? `${event.tagId}-${event.timestamp}-${eventIndex}`}`}
                className={`absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center transition-all duration-150 ${isCurrent ? "z-40" : isLatestVisible ? "z-30" : "z-10"}`}
              ...
              >
                <div
                  className={`flex items-center justify-center rounded-full border-2 border-white font-bold text-white shadow-lg ${isCurrent ? "h-5 w-5 bg-sky-600 px-2 text-[9px] ring-2 ring-sky-300" : isPastShadow ? "h-5 w-5 bg-sky-500/20 px-1 text-[7px] opacity-45 shadow-[0_0_10px_rgba(14,165,233,0.25)]" : "h-5 w-5 bg-sky-600 px-2 text-[9px]"}`}
                >
                  {isPastShadow ? "" : "T"}
                </div>
                {!isPastShadow && (
                  <span
                    className={`mt-1 max-w-28 truncate rounded bg-white/95 px-1.5 py-0.5 font-semibold text-slate-700 shadow ${isCurrent ? "text-[10px] text-sky-700" : "text-[9px]"}`}
                  >
                    {displayUserName}
                  </span>
                )}
        ...
        {tagId ? (
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                setPlaying(false);
                setAllHistoryIndex(Number(e.target.value));
              }}
            ...
            <p className="mt-1 text-xs text-slate-400">
              Move the timeline left to see older history. Maximum 10 past
              records are shown for each tag.
            </p>
          </div>
        )}
      </div>
...
function validFloorOptions(floors: Item[]): [string, string][] {
  return floors.flatMap((item) => {
    const id = floorId(item);
    if (id === undefined) return [];
    return [
      [
        String(id),
        str(item.name ?? item.floor_name ?? item.title, `Floor ${String(id)}`),
      ],
    ] as [string, string][];
  });
}
